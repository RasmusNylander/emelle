...
from linearRegression1 import gen_error_given_λ, regularised_linear_regression_model_weights, squared_error
...
def gen_error_given_hidden_units(data: ndarray, truth: ndarray, folds: KFold, hidden_units: ndarray) -> ndarray:
	...
	for k, (train_index, test_index) in tqdm(enumerate(folds.split(data, truth)), total=folds.n_splits, desc="ANN outer fold", unit="folds"):
